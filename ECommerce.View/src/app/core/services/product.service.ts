...
export class ProductService {
  ...
  getHomeData(context?: HttpContext): Observable<HomeData> {
    return this.cache.getOrFetch<HomeData>('homepage', 'home',
      (ifNoneMatch) => this.api.getWithHeaders<HomeData>("/home", { ifNoneMatch, context }).pipe(
        catchError(() => of({ data: this.fallbackHomeData, etag: null, cacheVersion: null, status: 200 }))
      )
    ).pipe(
  ...
  getHeroData(context?: HttpContext): Observable<any[]> {
    return this.cache.getOrFetch<any[]>('banners', 'hero',
      (ifNoneMatch) => this.api.getWithHeaders<any[]>("/home/hero", { ifNoneMatch, context }).pipe(
        catchError(() => of({ data: [], etag: null, cacheVersion: null, status: 200 }))
      )
    ).pipe(
      map(result => result.data),
      shareReplay(1)
  ...
  getNewArrivalsData(context?: HttpContext): Observable<Product[]> {
    return this.cache.getOrFetch<Product[]>('homepage', 'new-arrivals',
      (ifNoneMatch) => this.api.getWithHeaders<Product[]>("/home/products", { ifNoneMatch, context }).pipe(
        catchError(() => of({ data: [], etag: null, cacheVersion: null, status: 200 }))
      )
    ).pipe(
      map(result => result.data),
  ...
  getProducts(params?: any, context?: HttpContext): Observable<Pagination<Product>> {
    ...
    return this.cache.getOrFetch<Pagination<Product>>('productDetails', `list_${paramKey}`,
      (ifNoneMatch) => this.api.getWithHeaders<Pagination<Product>>(this.baseUrl, { params, ifNoneMatch, context })
    ).pipe(
      map(result => result.data)
    );
  }
  ...
  getFeaturedProducts(limit = 10, context?: HttpContext): Observable<Pagination<Product>> {
    return this.cache.getOrFetch<Pagination<Product>>('featuredProducts', 'all',
      (ifNoneMatch) => this.api.getWithHeaders<Pagination<Product>>(this.baseUrl, {
        params: { isFeatured: true, pageSize: limit },
        ifNoneMatch,
        context,
      }).pipe(
        catchError(() => of({ data: { data: [], count: 0 } as any, etag: null, cacheVersion: null, status: 200 }))
      )
    ).pipe(
      map(result => result.data),
      shareReplay(1)
  ...
  getBySlug(slug: string, context?: HttpContext): Observable<Product> {
    ...
    return this.cache.getOrFetch<Product>('productDetails', cacheKey,
      (ifNoneMatch) => this.api.getWithHeaders<Product>(`${this.baseUrl}/${slug}`, { ifNoneMatch, context })
    ).pipe(
      map(result => result.data)
    );
  }
  ...
  getReviewsByProductId(productId: number): Observable<Review[]> {
    ...
    return this.cache.getOrFetch<Review[]>('productReviews', cacheKey,
      (ifNoneMatch) => this.api.getWithHeaders<Review[]>(`/reviews/products/${productId}`, { ifNoneMatch })
    ).pipe(
      map(result => result.data)
    );
  }
