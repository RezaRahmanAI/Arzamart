/** @type {import('tailwindcss').Config} */
module.exports = {
  content: ["./src/**/*.{html,ts}"],
  darkMode: "class",
  theme: {
    extend: {
      colors: {
        brand: {
          blue: '#2f68ff',
          dark: '#111111',
          gray: '#a1a1aa',
          light: '#f4f4f5',
          green: '#046c4e',
          red: '#9b1c1c',
        },
        /* ── Design system tokens (new) ── */
        "ds-bg":           "var(--color-bg)",
        "ds-surface":      "var(--color-surface)",
        "ds-surface-2":    "var(--color-surface-2)",
        "ds-border":       "var(--color-border-ds)",
        "ds-text":         "var(--color-text-primary)",
        "ds-text-sec":     "var(--color-text-secondary)",
        "ds-text-muted":   "var(--color-text-muted)",
        "ds-accent":       "var(--color-accent-ds)",
        "ds-accent-hover": "var(--color-accent-hover)",
        "ds-hero-bg":      "var(--color-hero-bg)",
        "ds-hero-text":    "var(--color-hero-text)",
        "ds-primary":      "var(--color-primary)",
        "ds-danger":       "var(--color-danger)",
        "ds-danger-bg":    "var(--color-danger-bg)",
        "ds-success":      "var(--color-success)",
        "ds-success-bg":   "var(--color-success-bg)",
        "ds-warning":      "var(--color-warning)",
        "ds-warning-bg":   "var(--color-warning-bg)",
        "ds-info":         "var(--color-info)",
        "ds-info-bg":      "var(--color-info-bg)",
        "ds-whatsapp":     "var(--color-whatsapp)",
        "ds-messenger":    "var(--color-messenger)",
        "status-pending":    "var(--status-pending)",
        "status-confirmed":  "var(--status-confirmed)",
        "status-processing": "var(--status-processing)",
        "status-packed":     "var(--status-packed)",
        "status-shipped":    "var(--status-shipped)",
        "status-delivered":  "var(--status-delivered)",
        "status-cancelled":  "var(--status-cancelled)",
        "status-hold":       "var(--status-hold)",
        "status-preorder":   "var(--status-preorder)",
        "status-return":     "var(--status-return)",
        "status-refund":     "var(--status-refund)",
        "border-light":         "var(--color-border-ds)",
        "border-dark":          "#333333",
      },
      fontFamily: {
        sans:    ["Montserrat", "system-ui", "sans-serif"],
        display: ["Montserrat", "system-ui", "sans-serif"],
      },
      fontSize: {
        "ds-3xs":  ["0.5625rem",  { lineHeight: "1.5" }], // 9px
        "ds-2xs":  ["0.625rem",   { lineHeight: "1.5" }], // 10px
        "ds-xs":   ["0.6875rem",  { lineHeight: "1.5" }], // 11px
        "ds-sm":   ["0.8125rem",  { lineHeight: "1.5" }], // 13px
        "ds-base": ["0.9375rem",  { lineHeight: "1.6" }], // 15px
        "ds-lg":   ["1.0625rem",  { lineHeight: "1.6" }], // 17px
        "ds-xl":   ["1.25rem",    { lineHeight: "1.4" }], // 20px
        "ds-2xl":  ["1.375rem",   { lineHeight: "1.3" }], // 22px
        "ds-3xl":  ["1.625rem",   { lineHeight: "1.2" }], // 26px
        "ds-4xl":  ["2rem",       { lineHeight: "1.1" }], // 32px
        "ds-5xl":  ["2.5rem",     { lineHeight: "1.1" }], // 40px
      },
      spacing: {
        "ds-1":  "0.25rem",
        "ds-2":  "0.5rem",
        "ds-3":  "0.75rem",
        "ds-4":  "1rem",
        "ds-5":  "1.25rem",
        "ds-6":  "1.5rem",
        "ds-8":  "2rem",
        "ds-10": "2.5rem",
        "ds-12": "3rem",
        "ds-16": "4rem",
        "ds-20": "5rem",
        "ds-24": "6rem",
      },
      borderRadius: {
        DEFAULT: "var(--radius-md)",
        sm:   "var(--radius-sm)",
        md:   "var(--radius-md)",
        lg:   "var(--radius-lg)",
        xl:   "var(--radius-xl)",
        full: "var(--radius-full)",
      },
      transitionDuration: {
        DEFAULT: "150ms",
        fast: "150ms",
        base: "200ms",
      },
      maxWidth: {
        "site":    "1600px",
        "content": "1280px",
      },
    },
  },
  plugins: [
    require("@tailwindcss/forms"),
    require("@tailwindcss/container-queries"),
  ],
};
